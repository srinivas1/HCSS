...
List<UserVO> list = (List<UserVO>)request.getAttribute("userVO");
	...
	%>
	<table border="1">
	<tr><th>User Mail ID</th><th>Date Of birth</th><th>cell NO</th><th>First Name</th><th>select to delete</th></tr>
	<% 
	for(UserVO user:list){
	%>
	<tr>
	<td><%=user.getUserMailID()%></td><td><%=user.getUserDOB()%></td><td><%=user.getUserCellNo()%></td><td><%=user.getUserFName()%></td><td><form action="adminDeleteLockedUser.do"><input type="hidden" name="userMailID" value="<%=user.getUserMailID()%>"/><input type="submit" value="deleteUser"/></form></td>
	</tr>
<%
	}
%>
